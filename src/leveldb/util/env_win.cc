...
std::string& XREPPath(std::string& path);
std::wstring& XREPPath(std::wstring& path);
...
bool Win32Env::FileExists(const std::string& fname)
{
    ...
    std::wstring wpath;
	ToWidePath(XREPPath(path), wpath);
...
Status Win32Env::RenameFile( const std::string& src, const std::string& target )
{
	...
	ToWidePath(XREPPath(src_path), wsrc_path);
	std::string target_path = target;
    std::wstring wtarget_path;
	ToWidePath(XREPPath(target_path), wtarget_path);
...
Status Win32Env::NewRandomAccessFile( const std::string& fname, RandomAccessFile** result )
{
    Status sRet;
    std::string path = fname;
    Win32RandomAccessFile* pFile = new Win32RandomAccessFile(XREPPath(path));
    if(!pFile->isEnable()){
        delete pFile;
        *result = NULL;
        sRet = Status::IOError(path, Win32::GetLastErrSz());
...
Status Win32Env::NewLogger( const std::string& fname, Logger** result )
{
    Status sRet;
    std::string path = fname;
    Win32MapFile* pMapFile = new Win32MapFile(XREPPath(path));
...
Status Win32Env::NewWritableFile( const std::string& fname, WritableFile** result )
{
    Status sRet;
    std::string path = fname;
    Win32MapFile* pFile = new Win32MapFile(XREPPath(path));
    if(!pFile->isEnable()){
        *result = NULL;
