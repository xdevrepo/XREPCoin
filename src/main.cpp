...
#if defined(NDEBUG)
#error "XREP cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uXREP) are considered zero fee (for relaying and mining)
...
void UpdateCoins(const CTransaction& tx, CValidationState& state, CCoinsViewCache& inputs, CTxUndo& txundo, int nHeight)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        BOOST_FOREACH (const CTxIn& txin, tx.vin) {
            ...
            bool ret = inputs.XREPCoins(txin.prevout.hash)->Spend(txin.prevout, txundo.vprevout.back());
            assert(ret);
        }
    }

    // add outputs
    inputs.XREPCoins(tx.GetHash())->FromTx(tx, nHeight);
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        {
            CCoins outsEmpty;
            CCoinsModifier outs = view.XREPCoins(hash);
        ...
        if (!tx.IsCoinBase()) { // not coinbases because they dont have traditional inputs
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                CCoinsModifier coins = view.XREPCoins(out.hash);
                if (undo.nHeight != 0) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // XREP: We use certain sporks during IBD, so check to see if they are
